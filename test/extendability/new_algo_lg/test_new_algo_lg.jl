@testset "fastpdag2dag_lg" begin
	...
	@testset "Meek Rule R3" begin
		...
		out = fastpdag2dag_lg(g)
		@test nv(out) == 4 && ne(out) == 5 && has_edge(out, 1, 3) &&
			has_edge(out, 1, 4) && has_edge(out, 2, 1) && has_edge(out, 2, 3) &&
			has_edge(out, 4, 3)
	end
	...
	@testset "Meek Rule R4" begin
		...
		@test nv(out) == 4 && ne(out) == 5 && has_edge(out, 1, 2) &&
			has_edge(out, 1, 3) && has_edge(out, 1, 4) && has_edge(out, 3, 2) &&
			has_edge(out, 4, 3)
	end
	...
	@testset "More PDAGs with possible extensions" begin
		...
		out = fastpdag2dag_lg(input)
		@test nv(out) == 5 && ne(out) == 5 && has_edge(out, 1, 4) &&
			has_edge(out, 2, 3) && has_edge(out, 4, 5) &&
			has_edge(out, 5, 2) && has_edge(out, 5, 3)
	end
