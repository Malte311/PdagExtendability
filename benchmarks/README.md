...
and graphs with 10000 nodes each in the subfolder `n=10000`. The
...
There is a graph with 512 nodes in the subfolder `n=512`
and a graph with 1024 nodes in the subfolder `n=1024`.

All graphs are extendable (in fact, they are already DAGs).

### `complete-e`
This folder contains complete graphs.
There is a graph with 256 nodes in the subfolder `n=256`
and a graph with 512 nodes in the subfolder `n=512`.
...
and graphs with 512 nodes each in the subfolder `n=512`.
...
### `sparse-v2`
This folder contains sparse DAGs generated via the function
[`random_dag_v2`](https://github.com/Malte311/PdagExtendability/blob/master/src/utils/dag_generator.jl#L53).
The graphs in the subfolder `small` have 1000 nodes each and the
number of edges is given via `ne=...` in the filename.

Analogously, the subfolder `medium` contains graphs with 10000 nodes.
...
## `mpdirected` subdirectory
The `mpdirected` subdirectory contains maximally oriented partially directed
input graphs, i.e., graphs which are closed under the four Meek rules.

All graphs are extendable.

### `complete`
This folder contains complete graphs with 256 vertices in the subfolder
`n=256`. The graphs were taken from the `pdirected/complete/n=256/` directory
and converted into MPDAGs by applying Meek's rules exhaustively.

All graphs are extendable.

### `fromchordal`
This folder contains graphs generated from the chordal graphs in the directory
`pdirected/fromchordal/` by applying Meek's rules exhaustively.

All graphs are extendable.

### `path`
This folder contains path graphs with 10000 vertices in the subfolder
`n=10000`. The graphs were taken from the `pdirected/path/n=10000/` directory
and converted into MPDAGs by applying Meek's rules exhaustively.

All graphs are extendable.

### `sparse`
This folder contains sparse graphs with 10000 vertices in the subfolder
`n=10000`. The graphs were taken from the `pdirected/sparse/n=10000/` directory
and converted into MPDAGs by applying Meek's rules exhaustively.

All graphs are extendable.

### `star`
This folder contains star graphs with 10000 vertices in the subfolder
`n=10000`. The graphs were taken from the `pdirected/star/n=10000/` directory
and converted into MPDAGs by applying Meek's rules exhaustively.
...
There are graphs with 1000 nodes each in the subfolder `n=1000`
and graphs with 10000 nodes each in the subfolder `n=10000`. The
parameters are given by the filename. `p=...` is the probability
for an edge to be directed (i.e., `1-p` is the probability for
an edge to be undirected).
...
different. `p=...` is the probability for an edge to be directed
(i.e., `1-p` is the probability for an edge to be undirected).

Analogously, the subfolder `n=1024` contains graphs with 1024 nodes.
...
### `enumeration`
This folder contains graphs with very small amounts of undirected edges in order
to run enumeration algorithms with exponential runtime in the number of undirected
edges on them.

The subfolders `n=100`, `n=500`, and `n=1000` contain graphs with 100, 500, and
1000 nodes, respectively. The graphs are then separated by their number of
undirected edges: The subfolder `m=10` consists of graphs with exactly 10
undirected edges, `m=20` contains graphs with 20 undirected edges, and so on.

The complete graphs, path graphs, star graphs, and subtree graphs are extendable
while the other graphs are **not** extendable.
...
in fully undirected chordal graphs. The graphs in the subfolder `subtree-n=10000-k=1-1`
have been generated using the undirected chordal graph `subtree-n=10000-k=1-1` as a
...
Analogously, the subfolder `subtree-n=10000-k=2-1` contains graphs that were
generated using the undirected chordal graph `subtree-n=10000-k=2-1` as a
basis and the subfolder `subtree-n=10000-k=3-1` contains graphs that were
generated based on the undirected chordal graph `subtree-n=10000-k=3-1`.
...
graphs with 1024 nodes each. All instances represent the exact
same graph - only the numbering of the nodes is different.
The `p=...` in the filename gives the probability of an edge
...
There are graphs with 1000 nodes each in the subfolder `n=1000`
and graphs with 10000 nodes each in the subfolder `n=10000`. The
parameters are given by the filename. The `p=...` in the filename gives the
probability of an edge being directed (thus, `1-p` is the probability
of an edge being directed).
